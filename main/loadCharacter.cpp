...
double loadCharacter::retrieveMaxPoise() {
    ...
    cached = static_cast<double>(std::round(1000 * (max_head + max_chest + max_arm + max_leg) / .75));
    return cached;
}
...
std::pair<double, double> loadCharacter::negationsPoise(double equipLoad, const std::vector<double> &armorFraction,
                                                        double armorPercent, bool hasBullgoat,
                                                        const std::vector<std::vector<float> > &poiseArmorPieces) {
...
std::vector<std::vector<double> > effectiveHealth(int baseVigor, int baseEndurance, double armorPercent,
                                                  const std::vector<double> &armorFraction, int allocatedStatPoints,
                                                  bool hasBullgoat, bool hasGreatjar) {
    ...
    for (int i = 0; i <= allocatedStatPoints; i++) {
        ...
        potentialBuilds.push_back({
            computedEffectiveHp, poiseVal, static_cast<double>(hpIndex - baseVigor),
            static_cast<double>(enduranceIndex - baseEndurance)
        });
    }

    for (const auto &build: potentialBuilds) {
        for (int j = 0; j < DataParser::getPoiseSize(); j++) {
            if (build[1] >= DataParser::fetchPoise(j) && build[0] >= bestBreakPoints[j][0]) {
                bestBreakPoints[j] = build;
            } else if (build[1] < DataParser::fetchPoise(j)) {
                break;
            }
        }
    }
...
std::pair<std::vector<double>, double> bestNegations(double equipLoad) {
    ...
    opt.set_maxeval(500); //limit to prevent it from taking forever or breaking
...
double loadCharacter::bestEffectiveHP(int statPoints, const std::string &startingClass, bool hasGreatjar) {
    ...
    for (int i = 0; i < statPoints; i++) {
        ...
        {
            std::lock_guard<std::mutex> lock(negationsCacheMutex);
            auto &cache = hasGreatjar
                              ? starting_classes_negations_greatjar
                              : starting_classes_negations;
            if (cache.count(startingClass) && i < cache[startingClass].size()) {
                negation = cache[startingClass][i];
                precompute = true;
            }
        }
...

    {
        ...
        if (hasGreatjar && (!starting_classes_negations_greatjar.count(startingClass) || larger))
            starting_classes_negations_greatjar[startingClass] = negations;
    }

    return *std::max_element(effectiveHP.begin(), effectiveHP.end());
}
...
void rescaleClasses() {
    for (const auto &[name, stats]: starting_classes) {
        std::vector<double> imperfectStats = project(std::vector<double>(stats.begin(), stats.end()),
                                                     STARTING_CLASS_STAT_POINTS);
...
int loadCharacter::getMaxFPAshOfWar() {
    ...
    if (cached == -1) {
        ...
        cached = *std::max_element(fp.begin(), fp.end());
    }
    return cached;
}
...
/// Gets the lowest possible stamina cost for a weapon swing in the game
/// @return The lowest amount of stam a weapon could possibly consume
double loadCharacter::getMinStamCostWeapon() {
    static double cached = -1;
    if (cached == -1) {
        std::vector<double> stam;
        for (const auto &[id, data]: DataParser::retrieveAllWeapon()) {
            auto stamCost = DataParser::retrieveStaminaCost(id);
            if (stamCost > 0) stam.push_back(stamCost);
        }
        cached = *std::min_element(stam.begin(), stam.end());
    }
    return cached;
}
...
std::vector<Character> exponentialDecay(Character &characterInput, int delta) {
    ...
    for (int i = 0; i < NUM_ARMOR_PIECES; i++) {
        ...
        double weight = armorPiece.empty() ? 0.0 : loadCharacter::retrieveEquipWeight(armorPiece);
        //if no armor, weight is obviously 0
    ...
    double optimalEffectiveHp = loadCharacter::bestEffectiveHP(characterInput.getLevel(),
                                                               characterInput.getStartingClass(),
                                                               characterInput.getHasGreatjar());
    //original number of stat points allocated towards EHP
    int allocatedEhpStats = characterInput.getVigor() - baseClassVigor + characterInput.getEndurance() -
                            baseClassEndurance;
    ...
    std::vector<std::vector<double> > allocatedEhp = effectiveHealth(baseClassVigor, baseClassEndurance, armorPercent,
                                                                     armorFraction, allocatedEhpStats,
                                                                     characterInput.getHasBullgoat(),
                                                                     characterInput.getHasGreatjar());
    ...
    //"interesting" way to get the number of stat points
    {
        ...
        allocatedDamageStats += characterInput.getDamageStats()[charDamageStatIndex] - starting_classes.at(
            characterInput.getStartingClass())[i];
    }

    int allocatedMindStats = characterInput.getMind() - starting_classes.at(characterInput.getStartingClass())[
                                 CLASS_MIND_STAT_INDEX];
    ...
    for (auto statAllocation: statAllocationVariants) {
        ...
        newStatMlCharacter.setFpRatio(
            static_cast<double>(DataParser::fetchFp(mindIndex)) / loadCharacter::retrieveMaxFp(
                characterInput.getLevel(), characterInput.getStartingClass()));
        ...
        if (iter == stats_to_ehp.end()) {
            allocatedEhp = effectiveHealth(baseClassVigor, baseClassEndurance, armorPercent,
                                           armorFraction, statAllocation[1], characterInput.getHasBullgoat(),
                                           characterInput.getHasGreatjar());
        ...
        for (auto poiseBp: allocatedEhp) {
            ...
            double ehpDelta = ((poiseBp[0]) - characterInput.getEffectiveHealth()) / characterInput.
                              getEffectiveHealth();
...
std::vector<int> damageStatAllocation(const Character &characterInput, int damageStatPoints) {
    ...
    for (Weapon &weapon: characterInput.getWeapons()) {
        ...
        scalingAr.push_back(scalingData[index][0]);
    ...
    //svd setup
    auto matrixStats = convert_vvd_to_matrix(optimalStats);
    //converting an std::vector to an Eigen::MatrixXd which are basically the same thing for our purposes
    matrixStats.transposeInPlace();
    //transpose stat matrix so the optimal stat values for each weapon are now grouped together
...
void rankBuilds(const std::vector<std::vector<double> > &builds, const std::string &modelPath, int level,
                Character &characterInput, int numBuilds) {
    ...
    for (const auto &build: builds) {
        ...
        //removing the score value(why is this there in the first place?)
    ...
    Ort::Env env(ORT_LOGGING_LEVEL_WARNING, "rank");
    auto session = Ort::Session(env, w_modelPath.c_str(), session_options);

    Ort::AllocatorWithDefaultOptions allocator;
    auto input_name_alloc = session.GetInputNameAllocated(0, allocator);
    auto output_name_alloc = session.GetOutputNameAllocated(0, allocator);

    std::string input_name = input_name_alloc.get(); // needed for later things
    ...
    for (int i = 0; i < scoreData.size(); i++) {
        ...
        std::vector<float> buildData(mlStringBuilds.begin() + i * buildStringSize,
                                     mlStringBuilds.begin() + (i + 1) * buildStringSize);
    ...
    double best_ehp = loadCharacter::bestEffectiveHP(level, characterInput.getStartingClass(),
                                                     characterInput.getHasGreatjar());
    ...
    while (!buildPriorityQueue.empty()) {
        ...
        int mindStat = fpToMind(std::round(resultData[3] * max_fp)) - starting_classes[characterInput.
                           getStartingClass()][CLASS_MIND_STAT_INDEX];
        ...
        std::cout << "EHP is " << resultData[1] * best_ehp << ". Vigor invested is " << vigorStat <<
                ". Endurance invested is " << endStat << ". Mind invested is "
        ...
        std::cout << "Vig " << starting_classes[characterInput.getStartingClass()][0] + vigorStat << ", Mnd " <<
                starting_classes[characterInput.getStartingClass()][1] + mindStat
                << ", End " << starting_classes[characterInput.getStartingClass()][2] + endStat << ", Str " << stats[0]
                << ", Dex " << stats[1]
...
std::vector<std::vector<double> > createBuilds(Character characterInput, int level, int stride) {
    ...
    double bestEhp = loadCharacter::bestEffectiveHP(level, characterInput.getStartingClass(),
                                                    characterInput.getHasGreatjar());
    double swingValue = characterInput.getSwingValue();

    double minStamCost = loadCharacter::getMinStamCostWeapon();
    double maxStamLimit = DataParser::fetchStamina(99);
    double stamRatio = maxStamLimit / minStamCost;
    ...
    for (std::vector testValueSet: mesh) {
        ...
        if (ehpCache.find(testValueSet[1]) != ehpCache.end()) //check if key is present
        {
            ...
            effectiveHPs = effectiveHealth(baseVigor, baseEndurance, armorPercent, armorFraction, testValueSet[1],
                                           characterInput.getHasBullgoat(), characterInput.getHasGreatjar());
...
void loadCharacter::loadData() {
    ...
    rankBuilds(builds, "/home/sto/downscaling-optimization-main/soulsplanner-build-archive/models/rl90ish", 90,
               bloodsage, 2);
}
...
void loadCharacter::loadData(std::string buildJsonPath, int targetLevel, int buildsToProduce) {
    ...
    rankBuilds(builds, "/home/sto/downscaling-optimization-main/soulsplanner-build-archive/models/rl90ish", targetLevel,
               bloodsage, buildsToProduce);
}


std::mutex m;

void appendBuild(std::vector<Character> &characters, const std::string &outputFilePath) {
...
void loadCharacter::writeTrainingData(const std::string &trainingPath, const std::string &outputFilePath, int sl) {
    rescaleClasses();

    for (const auto &[name, stats]: starting_classes) {
        bestEffectiveHP(sl, name, false);
        bestEffectiveHP(sl, name, true);
    }
...
void loadCharacter::functionTesting() {
    ...
    auto projResult = project(std::vector<double>(starting_classes["Hero"].begin(), starting_classes["Hero"].end()),
                              79);
